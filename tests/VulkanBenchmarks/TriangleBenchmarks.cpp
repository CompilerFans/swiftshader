...
static void TriangleSolidColor(benchmark::State &state, Multisample multisample)
{
	...
	RunBenchmark(state, tester, "TriangleSolidColor", 1);
}
...
static void TriangleInterpolateColor(benchmark::State &state, Multisample multisample)
{
	...
	RunBenchmark(state, tester, "TriangleInterpolateColor", 1);
}
...
static void TriangleSampleTexture(benchmark::State &state, Multisample multisample)
{
	...
	tester.onUpdateDescriptorSet([](DrawTester &tester, vk::CommandPool &commandPool, vk::DescriptorSet &descriptorSet) {
		...
		Buffer buffer(physicalDevice, device, bufferSize, vk::BufferUsageFlagBits::eTransferSrc);
	...
	RunBenchmark(state, tester, "TriangleSampleTexture", 1);
}

static void ManySolidTriangles(benchmark::State &state, int triangleCount, Multisample multisample)
{
	DrawTester tester(multisample);
	...
	RunBenchmark(state, tester, "ManySolidTriangles", triangleCount);
}
...
BENCHMARK_CAPTURE(ManySolidTriangles, ManySolidTriangles_1K, 1024, Multisample::False)->Unit(benchmark::kMillisecond)->MeasureProcessCPUTime();
BENCHMARK_CAPTURE(ManySolidTriangles, ManySolidTriangles_16K, 16384, Multisample::False)->Unit(benchmark::kMillisecond)->MeasureProcessCPUTime();
BENCHMARK_CAPTURE(ManySolidTriangles, ManySolidTriangles_64K, 65536, Multisample::False)->Unit(benchmark::kMillisecond)->MeasureProcessCPUTime();
