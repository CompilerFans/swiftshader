...
- `vertex_dynamic_state`
  - the repository now has repo-local `vkCmdSetVertexInputEXT` coverage for a solid-color triangle, a multi-attribute interpolated-color triangle, and an `instance-rate` offset case; broader sample-style coverage still needs more formats/topologies.
  - 仓库内已经有 `vkCmdSetVertexInputEXT` 的纯色三角形、多 attribute 颜色插值三角形以及 `instance-rate` 偏移组合覆盖；更广的样例风格覆盖仍需补更多格式/拓扑。
- `hello_triangle_1_3`
...
- `render_passes`
  - Render pass basics exist, and repo-local coverage now includes `vkCmdClearAttachments` for both color and depth aspects, a `loadOp = LOAD` preservation case on presentable color attachments, and a narrow two-subpass overlay path using `vkCmdNextSubpass`; broader sample-driven validation for storeOp and more complex subpass/layout patterns is still missing.
  - render pass 基础已有，且仓库内已经覆盖了活动 render pass 内针对 color/depth 的 `vkCmdClearAttachments`、presentable color attachment 上 `loadOp = LOAD` 的保留语义，以及通过 `vkCmdNextSubpass` 的窄双 subpass 叠加路径；但 storeOp 和更复杂的 subpass/layout 模式仍待补齐。
- `msaa`
